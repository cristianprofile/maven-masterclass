...
public class EspecialidadDataOnDemand {
	...
	public void init() {
        int from = 0;
        int to = 10;
        data = especialidadRepository.findAll(new org.springframework.data.domain.PageRequest(from / to, to)).getContent();
        if (data == null) {
            throw new IllegalStateException("Find entries implementation for 'Especialidad' illegally returned null");
        }
        if (!data.isEmpty()) {
            return;
        }
